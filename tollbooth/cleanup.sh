...
if docker ps -a --format '{{.Names}}' | grep -qx casky-tollbooth; then
  docker rm -f casky-tollbooth >/dev/null && echo "  removed container: casky-tollbooth"
else
  echo "  no casky-tollbooth container found — already clean"
fi
