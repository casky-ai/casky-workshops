...
mkdir -p evidence   # this folder's own scratch output, gitignored — see evidence/README.md
...
{
  echo "=== HTTP/pcap traffic (SSRF -> IMDS credential leak) ==="; cat evidence/tollbooth-pcap.txt
  echo; echo "=== CloudTrail events (same account, following hours) ==="
  jq -c '.Records[]' cloudtrail/*.json
} > evidence/tollbooth-full.txt

# 3. OpenDoor's evidence is already text — just concatenate the configs
for f in opendoor/*.json; do echo "--- $(basename "$f") ---"; cat "$f"; echo; done > evidence/opendoor-full.txt
...
cp evidence/tollbooth-full.txt evidence/opendoor-full.txt ../casky-box/evidence/
