...
docker run -d --name casky-fastlane \
  --env-file .env \
  -v "$(pwd)":/root/fastlane \
  casky-fastlane

docker exec casky-fastlane test -f /root/fastlane/verify.sh \
  && echo "[+] mount OK" || echo "[!] wrong directory"

# 1. Run ./verify.sh — expect 12/12 PASS.
docker exec -w /root/fastlane casky-fastlane ./verify.sh

# 2. Bash into the container and start Claude Code interactively.
docker exec -it -w /root/fastlane casky-fastlane bash
